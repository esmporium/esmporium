...
This is why we have a search API for each different set of facet names i.e. query style.
For example, we have [SOLR_CMIP5][(m).] and [SOLR_CMIP6][(m).], rather than just SOLR.

[@Anna this might also help
...
So what we are effectively doing is adding the `search_cmip5` and `search_cmip6` functions ourselves
so that the user gets the experience which makes sense.
It just means we have to deal with the headache of making sure that the translation to `search_esgf_like` is correct,
and we are also trying to make it possible for the user to effectively still call `search_esgf_like` directly,
in case we get this translation wrong (this is what our `other_facets` escape hatch is for,
...
As a result, we have set this up so that we don't pass multiple sets of facet names in the one query.
...
Then show that, if you use `other_terms`, you can get them in the same query i.e. you can 'escape' if you want.
...
This gives you full control, but also the ability to shoot yourself in the foot (e.g. if you pass a facet that doesn't exist, you'll just get an error or no results and you have to figure out the error message yourself - we can't give nice error handling for paths that we (deliberately) can't predict).]
