...
def print_health_table(health: dict[str, HostHealth]) -> None:
    ...
    for h in sorted(health.values(), key=rank_by_response_time_median):
        time = (
            "-"
            if h.response_time_median == float("inf")
            else f"{h.response_time_median:.2f}s"
        )
...
def ranked_hosts(engine, project: str) -> list[str]:
    ...
    selector = build_health_selector(engine, rank=rank_by_response_time_median)
