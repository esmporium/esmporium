...
The name is chosen by hand rather than generated by `NAMING_CONVENTION`
(an explicitly-named index bypasses the convention,
which would otherwise spell out every column in the name).

The name has to agree in three places:
the model's `__table_args__`,
the migration that creates the index
and [`save_dataset`][esmporium.db.save_dataset],
which recognises a clash by finding this name
in the database's `IntegrityError` message.
Renaming it therefore needs a migration too.
"""
...
class DatasetRawDoc(EsmporiumBase, table=True):
    ...
    One document can describe several versions (e.g. CMIP5's raw documents).
