...
Given an explicit, stable name on purpose rather than letting `NAMING_CONVENTION` derive
one. Two places have to agree on this exact string: the index declared in
[`Dataset`][esmporium.db.schema.Dataset]'s `__table_args__`, and
[`esmporium.db.results_to_database.save_dataset`][], which recognises this name in the
`IntegrityError` a clash raises so it can re-raise a clearer error. A
convention-derived name would be a poor fit here for two reasons: it is built from the
column list, so adding a facet would silently rename the index and break that string
match; and the index covers a `coalesce(grid_label, '')` expression column, which has no
name for the convention to interpolate. A short constant the two sites share is the
clean way, so this is deliberately hard-coded rather than autogenerated.
"""
