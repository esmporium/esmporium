...
Last updated: 2026-08-21 (ok, this is also tracked in git, anyway)

## PR1: In progress,  see [TODO @Anna make PR and add link here]
...
- [x] conversion of query classes into ESGF1 and ESGF-NG query parameters
...
- [x] *deferred to PR1.5* check of search API performance database handling

Integration tests:

- [ ] hit the APIs using all query classes and check that we get the results we expect. Run this on all combinations, even e.g. CMIP5 with ESGF-NG and CMIP7 with ESGF1. We shouldn't get an error, we should just get no results.
...
- [ ] deferred to PR1. check that the API health stats appear in the expected database
    - do this both for ESGF1, ESGF15-bridge and ESGF-NG (don't worry about testing across all query classes though). This can just be part of the existing integration tests, no need to add new tests.
