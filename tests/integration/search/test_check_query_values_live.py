...
def only_report(reports: dict[str, ValueReport]) -> ValueReport:
    """
    Pull out the one report, or skip if no endpoint answered
...
def test_cmip5_experiment_typo_is_matched_to_the_real_spelling():
    reports = check_query_values(QueryCMIP5(experiment="abrupt-4xco2", variable="tas"))

    finding = finding_for(only_report(reports), "experiment")
...
def test_cmip6_variant_label_typo_is_flagged_against_the_published_values():
    ...
    finding = finding_for(only_report(reports), "variant_label")
