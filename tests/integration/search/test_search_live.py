...
def test_search_pages_through_all_the_results(
    ...
    try:
        outcome = search(
            ...
            limit=page_size,
            client=client,
            processor=lambda _host, parsed: pages.append(len(parsed)),
        )
    except NoAPIAnsweredError as exc:
        skip_or_fail(
            exc.failures,
    ...
    assert len(outcome.datasets[host]) == total, (
        f"{host} said {total} matched but paging collected "
        f"{len(outcome.datasets[host])} (the index may have shifted mid-scan)"
    )
    assert outcome.n_matches[host] == total
